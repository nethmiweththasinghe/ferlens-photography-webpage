export const reviews = [
  {
    name: "Sajitha & Ravina",
    occasion: "Birthday Photoshoot",
    text: `Dear Pasindu,
I want to thank you for the incredible photos you took of our baby’s birthday.
Your calm and friendly approach made everything so easy and enjoyable. You went above and beyond to make us feel comfortable, and it truly shows in the stunning photos which we’ll cherish for a lifetime. Thank you so much again!`,
    image: "/photos/reviews/SajithaAndRaveena.jpeg",
  },
  {
    name: "Asako and Aidan",
    occasion: "Portrait",
    text: `Super professional shoot, photos came out just like we wanted - very loyal to the references.
The photographer was very patient with requests and suggested alternative shots when he thought would be an improvement. 100% recommend!`,
    image: "/photos/reviews/Asako.jpeg",
  },
  {
    name: "Ishani & Sachinthaka",
    occasion: "Maternity Photoshoot",
    text: "Thank you so much Fern Lens, Pasindu malli for capturing our maternity shoot. We are really happy choosing you. Thank you for going extra mile to make sure everything turns out beautifully, and your dedication really shows in the work. We’re very happy with the experience and the final captures. Absolutely affordable and worth it. Highly recommend!!! ",
    image: "/photos/reviews/Ishani.jpeg",
  },
  {
    name: "Sewmini Gunawardhana",
    occasion: "Graduation Ceremony",
    text: "Absolutely loved my graduation photos! Amazing work, great attention to detail and such a comfortable experience. Highly recommend his work!",
    image: "/photos/reviews/sewmini.jpeg",
  },
  {
    name: "Thilanka Rathnayake",
    occasion: "Graduation Ceremony",
    text: `I had such a great experience with Pasindu. From the start, they were really easy to talk to and made everything feel relaxed and natural.
The photos turned out even better than I expected. He captured moments so naturally, and you can tell they really pay attention to the little details. Nothing felt forced or overly edited, just genuine, beautiful shots.
I also got the photos back pretty quickly, which was a nice bonus. Overall, I’m really happy with how everything turned out and would definitely recommend them to anyone looking for a photographer. ♥️`,
    image: "/photos/reviews/Thilanka.jpeg",
  },
  {
    name: "Ramod Kushan",
    occasion: "Graduation Ceremony",
    text: "I’m really happy with my graduation photos! Pasindu captured everything so perfectly and made the whole experience comfortable and enjoyable. The photos look amazing and feel so natural. I would definitely recommend Fern Lens Photography to anyone looking for quality work!",
    image: "/photos/reviews/Ramod.jpeg",
  },
];
